...
async function openApp(profile) {
    ...
    try { await loadCourts(); await renderCourts(); await renderReservations(); await loadNotifications(); renderAdminCourts(); } catch (error) { showToast(error.message); }
}
...
$('#googleButton').addEventListener('click', async () => { const { error } = await supabaseClient.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: authRedirectUrl } }); if (error) showToast(error.message); });
...
$('#prevDay').addEventListener('click', async () => { currentDate.setDate(currentDate.getDate() - 1); selectedCourt = null; selectedTime = null; updateRecurrenceOptions(); await renderCourts(); updateSummary(); });
