...
$('#courtAdminList').addEventListener('click', async (event) => { const button = event.target.closest('.edit-court'); if (!button) return; const court = courts.find((item) => item.id === Number(button.dataset.courtId)); const name = window.prompt('Nome da quadra:', court.name); if (!name?.trim()) return; const surface = window.prompt('Tipo de piso:', court.surface); const location = window.prompt('Localização:', court.location); const { error } = await supabaseClient.from('courts').update({ name: name.trim(), surface: surface?.trim() || court.surface, location: location?.trim() || court.location }).eq('id', court.id); if (error) { showToast(error.message); return; } showToast('Dados da quadra atualizados.'); await loadCourts(); await renderCourts(); renderAdminCourts(); });
