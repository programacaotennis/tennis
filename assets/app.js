...
async function loadNotifications() { const { data } = await supabaseClient.from('notifications').select('id, message, court_id, booking_date, start_time').eq('user_id', currentUser.id).eq('read', false).order('created_at', { ascending: false }).limit(20); const panel = $('#notificationList'); if (data?.length) { $('#notificationButton').classList.add('has-notifications'); $('#notificationButton').title = `${data.length} nova(s) notificação(ões)`; panel.innerHTML = data.map((notification) => `<button class="notification-item" data-notification-id="${notification.id}" data-court-id="${notification.court_id || ''}" data-date="${notification.booking_date || ''}" data-time="${notification.start_time?.slice(0, 5) || ''}"><strong>Horário liberado</strong><span>${notification.message}</span></button>`).join(''); } else { panel.innerHTML = '<p class="notification-empty">Nenhuma notificação nova.</p>'; } }
...
async function openApp(profile) {
    ...
    try { await loadCourts(); await loadAvailability(); await renderCourts(); await renderReservations(); await loadNotifications(); renderAdminCourts(); if (profile.role === 'admin') { await renderAdminStats(); await renderMembers(); } } catch (error) { showToast(error.message); }
}
