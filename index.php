...
<body>
    ...
    <main class="app-shell hidden" id="appScreen">
        ...
        <section class="content-area">
            <header class="topbar"><div class="mobile-brand"><img class="app-logo" src="src/img/ico.png" alt=""> Programação Tênis</div><div class="topbar-actions"><button class="icon-button" id="notificationButton" title="Notificações" aria-label="Abrir notificações"><svg class="notification-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M3.5 6.5h17v11h-17z"/><path d="m4 7 8 6 8-6"/></svg></button><div class="user-menu"><div class="avatar" id="userAvatar">VC</div><div><strong id="userName">Visitante</strong><small id="userRole">Membro</small></div></div></div></header>
            ...
            <div class="notification-panel hidden" id="notificationPanel"><div class="notification-panel-header"><strong>Notificações</strong><div><button type="button" class="clear-notifications" id="clearNotificationsButton">Limpar</button><button type="button" id="closeNotifications" aria-label="Fechar notificações">×</button></div></div><div id="notificationList"></div><div class="notification-panel-footer"><button type="button" id="enablePushButton">Ativar alertas neste dispositivo</button></div></div>
            <div class="view active-view" id="bookingView">
                <div class="page-intro"><div><p class="eyebrow">RESERVE SEU MOMENTO</p><h2>Escolha quando jogar.</h2><p>Selecione uma quadra e um horário disponível para confirmar sua partida.</p></div><div class="date-control"><button id="prevDay">‹</button><button class="date-display" id="openCalendar" type="button"><small>DATA DA RESERVA</small><strong id="selectedDateLabel">Hoje</strong><span class="calendar-icon" aria-hidden="true">▣</span></button><input id="datePicker" type="date" aria-label="Escolha a data da reserva"><button id="nextDay">›</button></div></div>
            ...
            <div class="view" id="adminView"><div class="page-intro compact"><div><p class="eyebrow">ÁREA RESTRITA</p><h2>Painel do clube.</h2><p>Uma visão rápida da operação de hoje.</p></div><div><button class="outline-button" id="changeAccessButton">Alterar acesso</button><button class="primary-button small" id="newCourtButton">＋ Nova quadra</button></div></div><div class="admin-grid"><div class="stat-card"><span>RESERVAS HOJE</span><strong id="bookingCount">12</strong><small class="positive">↑ 18% esta semana</small></div><div class="stat-card"><span>QUADRAS ATIVAS</span><strong id="activeCourtCount">00</strong><small>todas operacionais</small></div><div class="stat-card"><span>TAXA DE OCUPAÇÃO</span><strong id="occupancyRate">0%</strong><small>últimos 7 dias</small></div></div><div class="admin-section"><div class="section-heading"><div><h3>Quadras e disponibilidade</h3><p>Configure os horários que os membros podem reservar.</p></div><span class="table-date">Disponibilidade diária</span></div><div class="court-admin-list" id="courtAdminList"></div></div><div class="admin-section members-section"><div class="section-heading"><div><h3>Usuários e administradores</h3><p>Escolha quem pode gerenciar o clube.</p></div></div><div class="member-list" id="memberList"></div></div></div>
        </section>
    </main>
    <div class="court-modal hidden" id="courtModal" role="dialog" aria-modal="true" aria-labelledby="courtModalTitle"><div class="court-modal-card"><div class="court-modal-header"><div><p class="eyebrow">CONFIGURAÇÃO</p><h3 id="courtModalTitle">Nova quadra</h3><p>Informe os dados que serão exibidos aos membros.</p></div><button class="modal-close" id="closeCourtModal" type="button" aria-label="Fechar">×</button></div><form id="courtForm"><label for="courtName">Nome da quadra</label><input id="courtName" type="text" placeholder="Ex.: Quadra Central" required maxlength="60"><div class="modal-form-grid"><div><label for="courtSurface">Tipo de piso</label><select id="courtSurface"><option>Saibro</option><option>Rápida</option><option>Grama</option></select></div><div><label for="courtLocation">Localização</label><select id="courtLocation"><option>Externa</option><option>Coberta</option><option>Indoor</option></select></div></div><label class="modal-check"><input id="courtActive" type="checkbox" checked><span>Disponível para reservas</span></label><div class="court-modal-actions"><button class="danger-outline-button hidden" id="deleteCourtButton" type="button">Excluir quadra</button><button class="outline-button" id="cancelCourtModal" type="button">Cancelar</button><button class="primary-button" id="saveCourtButton" type="submit">Salvar quadra <span>→</span></button></div></form></div></div><div class="toast" id="toast" role="status"></div>
    <div class="court-modal cancel-booking-modal hidden" id="cancelBookingModal" role="dialog" aria-modal="true" aria-labelledby="cancelBookingTitle"><div class="court-modal-card cancel-booking-card"><span class="cancel-booking-icon" aria-hidden="true">!</span><p class="eyebrow">LIBERAR HORÁRIO</p><h3 id="cancelBookingTitle">Desistir desta reserva?</h3><p>Este horário ficará disponível para outros membros e não poderá ser recuperado automaticamente.</p><div class="court-modal-actions"><button class="outline-button" id="keepBookingButton" type="button">Voltar</button><button class="danger-button" id="confirmCancelBooking" type="button">Liberar horário <span>→</span></button></div></div></div>
